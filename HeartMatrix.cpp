...
char HeartMatrix :: get_user_char(){
	return (selected_index+32);
}
...
void HeartMatrix :: displayer() {
	...
	if(cur_mode == 2){
      ...
      if ( cur_col == 0 )write_byte(1, font_5x7[selected_index][cur_col]);
      else if (cur_col == 1)write_byte(0, font_5x7[selected_index][cur_col ]);
	  else write_byte(cur_col, font_5x7[selected_index][cur_col]);
      write_byte(cur_col+5, 0x00);
		/*
